...
public:
	...
	void ft_recv(std::vector<std::string> msg)
	{
		...
		if (msg.size() < 3)
		{
			...
			this->_send_msg = ERR_NOSUCHNICK(this->_client, "");
			this->ft_append_msg();
			return;
		}
		std::vector<std::string> nickname;
		nickname = split(msg[2], ",");
		for (size_t i = 0; i < nickname.size(); i++)
		{
			if (!this->_server->ft_get_nick(nickname[i]))
			{
				this->ft_set_client("401");
				this->_send_msg = ERR_NOSUCHNICK(this->_client, nickname[i]);
				this->ft_append_msg();
			}
			else if (this->_user->ft_get_channel(msg[1])->ft_get_user_list().find(this->_server->ft_get_nick(nickname[i])->ft_get_user_name()) == this->_user->ft_get_channel(msg[1])->ft_get_user_list().end())
			{
				this->ft_set_client("441");
				this->_send_msg = ERR_USERNOTINCHANNEL(this->_client, nickname[i], msg[1]);
				this->ft_append_msg();

			}
			else
			{
				this->_send_msg = this->_user->ft_get_info() + " " + this->_cmd + " " + msg[1] + " " + nickname[i] + " ";
				...
				this->_user->ft_get_channel(msg[1])->ft_channel_leave_user(this->_server->ft_get_nick(nickname[i]));
			}
		}
	}
};
