...
public:
	...
	void ft_recv(std::vector<std::string> msg)
	{
		...
		if (msg.size() > 1)
			channel = split(msg[1], ",");
		if (msg.size() > 2)
			password = ft_split_join_param(msg[2], ",");
		size_t i = 0;
		while (i < channel.size())
		{
			...
			else if (code == 0)
			{
				...
				if (this->_server->ft_get_channel(channel.at(i))->ft_get_topic() != "")
				{
					...
					this->_send_msg = RPL_TOPIC(this->_client, channel.at(i), this->_user->ft_get_channel(channel.at(i))->ft_get_topic());
					this->ft_append_msg();
					this->ft_set_client("333");
					this->_send_msg = RPL_TOPICWHOTIME(this->_client, channel.at(i),
													this->_server->ft_get_channel(channel.at(i))->ft_get_topic_user()->ft_get_nick_name(),
													this->_server->ft_get_channel(channel.at(i))->ft_get_topic_time());
				}
				if (this->_server->ft_get_channel(channel.at(i))->ft_mode_has())
				{
					this->ft_set_client("324");
					this->_send_msg = RPL_CHANNELMODEIS(this->_client, channel.at(i), this->_server->ft_get_channel(channel.at(i))->ft_mode_string());
					this->ft_append_msg();
				}
			}
			i++;
